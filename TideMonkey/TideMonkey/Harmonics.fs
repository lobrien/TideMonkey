...
type ConstantsT = { Index : int; Name : string; Phase : float<Degrees>; Amp : float }
...
type RefT = { StationId : string; M2Phase : float; }
...
type EquilibriumT = { Constituent : int; Year : Year; Value : float<Degrees> }

type NodeFactorT = { Constituent : int; Year : Year; Value : float } 
...
module Harmonics = 
    ...
    let ConstantsFromTableElement (el : XElement) = 
        try
            { 
                ...
                Phase = el.Element(xn "phase").Value |> float |> (*) 1.0<Degrees>
    ...
    let Equilibria = 
        ...
        { 
            ...
            Value = el.Element(xn "value").Value |> float |> fun f -> f * 1.0<Degrees>; 
        }) 
    ...
    let DataSetFromTableElement (el : XElement) = 
        ...
        try
            ...
            let minDir = 
                OptionEl "min_dir" float 
                |> Option.bind (fun f -> 
                    f * 1.0<Degrees> 
                    |> fun d -> { Degrees = d ; IsDegreesTrue = true } 
                    |> Some) 
            let maxDir = 
                OptionEl "max_dir" float 
                |> Option.bind (fun f -> 
                    f * 1.0<Degrees> 
                    |> fun d -> { Degrees = d ; IsDegreesTrue = true }
                    |> Some) 
    ...
    let Refs = ReadFromFile refsFile (fun el -> 
        try 
            {
    ...
    *)
    ...
    let StationFrom (dbh : DbHeaderT) (ds : DataSetT) (constants : ConstantsT seq) (constituentsData : ConstituentsDataT seq) (simpleOffset : SimpleOffsetsT) = 
        ...
        let stationTypePrefix = match isReferenceStation with
        ...
        let stationTypeText = 
            stationTypePrefix + 
            match (isCurrent, isHydraulicCurrent) with
            | (false, _) -> "tide"
        ...
        let datum = { Amplitude = { Value = ds.Datum.Value; Units = ds.Units } }
