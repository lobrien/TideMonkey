...
type SunMoonEventsT = 
   { Sunrises : DateTime list
     Sunsets : DateTime list
     Moonrises : DateTime list
     Moonsets : DateTime list
     MoonPhases : (DateTime * MoonPhaseT) list }

// DeltaPhi = Nutation in Longitude, DeltaEpsilon = nutation in obliquity
type NutationT = 
   { DeltaPsi : float<ArcSeconds>
     DeltaEpsilon : float<ArcSeconds> }

type SiderealTimeT = 
   { MeanSiderealTime : HMS
     ApparentSiderealTime : HMS }

type EphemerisEntryT = 
   { Date : DateTime
     RightAscension : float<Degrees>
     Declination : float<Degrees>
     ApparentSiderealTime : float<DecimalHours> }

type EphemerisT = 
   { Prior : EphemerisEntryT
     Day : EphemerisEntryT
     Following : EphemerisEntryT }

type RisingTransitSettingT = 
   { Rising : DateTime
     Transit : DateTime
     Setting : DateTime }
...
module Calendar = 
   let ToJulianDate(dateTime : DateTime) = dateTime.ToOADate() + 2415018.5
...
module Ephemeris = 
   let private EphemerisData solarOrLunar = 
      ...
      let rs = 
         ...
         |> Seq.map 
               (fun (julianDate, ra, dec, decimalHours) -> 
               (float (julianDate)), float (ra), float (dec), float (decimalHours))
         |> Seq.map 
               (fun (jd, ra, dec, localSiderealTime) -> 
               (jd, ra * 1.0<Degrees>, dec * 1.0<Degrees>, localSiderealTime * 1.0<DecimalHours>))
         |> Seq.map (fun (jd, ra, dec, localSiderealTime) -> 
               { Date = Calendar.FromJulianDate(jd)
                 RightAscension = ra
                 Declination = dec
                 ApparentSiderealTime = localSiderealTime })
      rs
...
module Meeus = 
   ...
   let JDE k T = 
      2451550.09766 + 29.530588861 * k + 0.00015437 * T * T + 0.000000150 * T * T * T + 0.00000000073 * T * T * T * T
   ...
   let M T k = 2.5534 + 29.10535670 * k - 0.0000014 * T * T - 0.00000011 * T * T * T |> (*) 1.<Degrees>
   //Moon's mean anomaly (49.5)
   let M' T k = 
      201.5643 + 385.81693528 * k + 0.0107582 * T * T + 0.00001238 * T * T * T - 0.000000058 * T * T * T * T 
      |> (*) 1.<Degrees>
   ...
   let Omega T k = 124.7746 - 1.56375588 * k + 0.0020672 * T * T + 0.00000215 * T * T * T |> (*) 1.0<Degrees>
   ...
   let ApparentPhaseNonPlanetaryCorrections moonPhase (e : float) (m : float<Degrees>) (m' : float<Degrees>) 
       (f : float<Degrees>) (omega : float<Degrees>) = 
      let E = e
      let M = (normalize m)
      let M' = (normalize m')
      let F = (normalize f)
      ...
      let constantsAndOps = 
         ...
         | FirstQuarter | LastQuarter -> List.zip quarterConstants quarterOps
      ...
      let w = 
         0.00306 - 0.00038 * E * cos (deg2rad (M)) + 0.00026 * cos (deg2rad (M')) - 0.00002 * cos (deg2rad (M' - M)) 
         + 0.00002 * cos (deg2rad (M' + M)) + 0.00002 * cos (deg2rad (2. * F))
      
      let correctedForQuarter = 
         match moonPhase with
         | FirstQuarter -> w
         | LastQuarter -> -w
         | NewMoon | FullMoon -> 0.0
   ...
   let NutationApprox date = 
      ...
      let deltaPsi = 
         let components = List.zip deltaPsiCoefficients opsPsi |> List.map (fun (a, b) -> a * b)
         components
         |> List.sum
         |> normalizeRadians
         |> rad2arcsec
      ...
      let deltaEpsilon = 
         let components = List.zip deltaEpsilonCoefficients opsEps |> List.map (fun (a, b) -> a * b)
         components
         |> List.sum
         |> normalizeRadians
         |> rad2arcsec
   ...
   let TrueObliquityOfEcliptic date = 
      ...
      let coeffs = 
         [ { Degrees = 23<Degrees>
             Minutes = 26<ArcMinutes>
             Seconds = 21.448<ArcSeconds> }
           { Degrees = 0<Degrees>
             Minutes = 0<ArcMinutes>
             Seconds = -46.8150<ArcSeconds> }
           { Degrees = 0<Degrees>
             Minutes = 0<ArcMinutes>
             Seconds = -0.00059<ArcSeconds> }
           { Degrees = 0<Degrees>
             Minutes = 0<ArcMinutes>
             Seconds = 0.001813<ArcSeconds> } ]
         |> List.map dms2deg
   ...
   let AltAz localHourAngle observersLatitude declination = 
      ...
      let Azimuth = 
         sin H / (cos (H) * sin (phi) - tan (delta) * cos (phi))
         |> atan
         |> fun r -> rad2deg (r * 1.0<Radians>)
   ...
   let RisingAndSetting coordinates ephemeris h0 = 
      ...
      if element < -1.0 || element > 1.0 then raise (Exception("NotImplementedException -- circumpolar body"))
      //Approximate time 
      let H0 = 
      ...
      let TransitCorrection m isTransit = 
         ...
         let localHourAngle = siderealTimeInGreenwich - longitude - alpha |> NormalizeHalfCircle
      ...
      let correctionValues = 
         [ (m0, true)
           (m1, false)
           (m2, false) ]
         |> List.map (fun (m, isTransit) -> TransitCorrection m isTransit)
         |> Array.ofList
      ...
      let transitDateTime = 
         midnight.AddHours(float transitTime.Hours).AddMinutes(float transitTime.Minutes)
                 .AddSeconds(transitTime.Seconds)
      let risingDateTime = 
         midnight.AddHours(float risingTime.Hours).AddMinutes(float risingTime.Minutes).AddSeconds(risingTime.Seconds)
      let settingDateTime = 
         midnight.AddHours(float settingTime.Hours).AddMinutes(float settingTime.Minutes)
                 .AddSeconds(settingTime.Seconds)
      { Rising = risingDateTime
        Transit = transitDateTime
        Setting = settingDateTime }
...
module Moon = 
   let PhasesForMoonCycle(t : DateTime) = 
      ...
      let ks = 
         [ (Math.Floor(ka), NewMoon)
           (Math.Floor(ka) + 0.25, FirstQuarter)
           (Math.Floor(ka) + 0.5, FullMoon)
           (Math.Floor(ka) + 0.75, LastQuarter) ]
      ks
